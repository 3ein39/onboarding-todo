...
import { ContactModelUtil } from '@/models/contact/contact.modelUtil'
...
const confirmDeleteDialog = useVcDialog({
  component: () => import('@/components/app/dialog/AppConfirmDialog.vue'),
})

const fullName = computed<string | null>(() => ContactModelUtil.getFullName(props.contact))
...
function onDeleteContact(): void {
  confirmDeleteDialog.open({
    ...
    description: i18n.t('module.contact.delete.confirm.description', {
      name: fullName.value ?? props.contact.email,
    }),
    onConfirm: async () => {
...
<template>
  <AppPage :title="fullName ?? contact.email ?? i18n.t('module.contact.unknown')">
    <template #actions>
      <VcButton
